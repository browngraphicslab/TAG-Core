...
rin.internal.DoubleAnimation = function (duration, from, to) {
    this.duration = (duration === 0) ? duration : duration || this.duration;
    this.from = from;
    this.to = to;
    this.keyframe = null;
            ...
            func = function () {
                ...
                if (!rin.internal.systemResourcesProcessed) {
                    promise = self.parallelLoader([
                            { src: rin.util.combinePathElements(systemRoot, 'lib/jquery-1.9.1.min.js'), loadChecker: function () { return window.jQuery !== undefined; } },
                            { src: rin.util.combinePathElements(systemRoot, 'lib/knockout-2.2.1.js'), loadChecker: function () { return window.ko !== undefined; } }
                    ])
                    .then(function () {
...
rin.internal.Orchestrator.prototype = {
    ...
    _onScreenPlayEnded: function () {
        ...
        if (configuration.loop) {
            ...
            this.pause(this._narrativeInfo.totalDuration + 0.1);
...
rin.internal.OrchestratorProxy.prototype = {
    ...
    getESItems: function () {
        return this._orchestrator.esItemsManager.screenPlayInterpreter.getESItems();
    },
...
rin.internal.DefaultScreenPlayInterpreter.prototype = {
    ...
    initialize: function (screenPlayId, segmentData, orchestrator) {
        ...
        for (var i = 0, len = experienceStreamReferenceList.length; i < len; i++) {
            ...
            // bleveque: added the below to check if there are any other exp streams with same id (i.e. same track) -- in that case, use the same zIndex
            var existSameTrack = { bool: false, zIndex: 0 };
            for (var ii = 0; ii < esItems.length; ii++) {
                if (esItems[ii].id === esData.id) {
                    existSameTrack.bool = true;
                    existSameTrack.zIndex = esItems[ii].zIndex;
                    break;
                }
            }
            var finalZIndex = 0;
            if (existSameTrack.bool)
                finalZIndex = existSameTrack.zIndex;
            else
                finalZIndex = parseInt(experienceStreamReference.zIndex) || lastZIndex++;
            // end bleveque
...
rin.internal.DefaultBufferingES.elementHTML = "<div id='bufferingDiv' style='margin:auto;width:100%;height:100%;font-size:18px;color:white;display:table;background:rgba(0,0,0,0.1);z-index:10000000;'><div style='text-align:center;vertical-align: middle;display:table-cell;'><div style='width:auto;height:auto;clear:right;margin-left:auto;margin-right:auto;margin-bottom:8px;'><img class='loaderGIFContainer' /></div>Loading...</div></div>";
/*!
* RIN Experience Provider JavaScript Library v1.0
* http://research.microsoft.com/rin
*
...
rin.internal.RinDebugger = function (rinPlayer) {
    ...
    setInterval(function () {
        if (rinPlayer.orchestrator && rinPlayer.orchestrator.getPlayerState() === rin.contracts.playerState.pausedForBuffering) {
            ...
            self.getPreloaderBufferingESs().foreach(function (es) { log += es.id + ", " });

...
function createDisp (dispNum, dispStartTime, dispLength) {
	...
	while (newKeyframe) {
		...
		if (tempNextTime === dispEndTime){
			...
			prop = {"sizeX":getRandomInt (30, 100), "sizeY":getRandomInt (30, 100),"posX":getRandomInt (-100, 100),"posY":getRandomInt (-100, 100)};
		}
	}
...
function createTour (options, length, tourName, tourAssets) {
	...
	for (i=1; i<imageNum; i++) {
		myTour += track + ", ";
		track = createImageTrack("Track"+i, length, tourAssets[i%tourAssets.length]);
	}
	if (imageNum > 1) {
		myTour += track;
	}
...
var tourAssets = ["minionBananas.png", "minionHole.png", "minionMoreBananas.png", "minionThinking.png", "minionWriting.png"];
